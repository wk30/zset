...
func TestNew(t *testing.T) {
	...
	rank, score, extra = s.GetRank("", false)
	if rank == -1 {
...
func TestIncrBy(t *testing.T) {
	z := New()
	for i := 1000; i < 1100; i++ {
		z.Set(float64(i), strconv.Itoa(i), "Hello world")
	}
...
func TestRange(t *testing.T) {
	...
	ids := make([]string, 0, 6)
	z.Range(0, -1, func(score float64, k string, _ interface{}) {
		ids = append(ids, k)
		t.Log(score, k)
	})
	if ids[0] != "1001" ||
	...
	z.RevRange(1, 3, func(score float64, k string, _ interface{}) {
		t.Log(score, k)
	})
...
func BenchmarkSortedSet_Add(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		s.Set(scores[i], strconv.Itoa(IDs[i]), nil)
	}
}
...
func BenchmarkSortedSet_GetRank(b *testing.B) {
	l := s.Length()
	for i := 0; i < b.N; i++ {
		s.GetRank(strconv.Itoa(100000+i%int(l)), true)
	}
}
